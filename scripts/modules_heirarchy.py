...
parser.add_argument('--e', help="Space separated list of external dependencies (eg ci environments rely on networking)",
                    nargs="*")
    ...
    for filename in [f for f in filenames if f == 'remote_states.tf']:
        with open(os.path.join(dir_path, filename), "r") as my_file:
            ...
            for output in outputs["data"].values():
                ...
                if '/origins/' in dir_path:
                    origins.append(module_name)
                else:
